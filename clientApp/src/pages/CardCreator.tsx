...
export default function CardCreatorPage(props: CardCreatorPageProps) {
  ...
  const default_subtitle =
    getFromLocalStorage<string>(SUBTITLE_KEY) ?? "ЯБЛОКО";
  const default_description =
    getFromLocalStorage<string>(DESCRIPTION_KEY) ??
    "Eating an apple a day keeps the doctor away";

  const default_titleColor =
    getFromLocalStorage<string>(TITLE_COLOR_KEY) ?? "db1b51";
  const default_subtitleColor =
    getFromLocalStorage<string>(SUBTITLE_COLOR_KEY) ?? "24de55";
  const default_descriptionColor =
    getFromLocalStorage<string>(DESCRIPTION_COLOR_KEY) ?? "000000";

  const default_externalFrameColor =
    getFromLocalStorage<string>(EXTERNAL_FRAME_COLOR_KEY) ?? "ff00aa";
  const default_internalFrameColor =
    getFromLocalStorage<string>(INTERNAL_FRAME_COLOR_KEY) ?? "99ff99";

  const default_imageSearch =
    getFromLocalStorage<string>(IMAGE_SEARCH_KEY) ?? "apple";
  const default_imageUrl =
    getFromLocalStorage<string>(IMAGE_URL_KEY) ??
    "https://cdn-icons-png.flaticon.com/128/17414/17414134.png";
  ...
  const [subtitleColor, setSubtitleColor] = React.useState(
    default_subtitleColor
  );
  const [descriptionColor, setDescriptionColor] = React.useState(
    default_descriptionColor
  );

  const [externalFrameColor, setExternalFrameColor] = React.useState(
    default_externalFrameColor
  );
  const [internalFrameColor, setInternalFrameColor] = React.useState(
    default_internalFrameColor
  );
      ...
      <Grid container style={{ marginTop: 32 }}>
        ...
        >
          <TextEditor
            ...
            subtitleColor={subtitleColor}
            setSubtitleColor={(event) =>
              setSubtitleColor(event.value as string)
            }
            descriptionColor={descriptionColor}
            setDescriptionColor={(event) =>
              setDescriptionColor(event.value as string)
            }
          />
          <FrameOptions
